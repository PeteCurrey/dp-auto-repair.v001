...
const TestimonialsSection = () => {
  ...
  return (
    <section className="py-16 bg-secondary text-secondary-foreground relative grain-texture">
      <div className="container mx-auto px-4 relative z-10">
        {/* Trust Badges */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16">
          {trustBadges.map((badge, i) => (
            <ScrollReveal key={badge.label} delay={i * 100} direction="scale">
              <div className="flex flex-col items-center text-center p-6 rounded-xl bg-secondary-foreground/5 backdrop-blur-sm border border-secondary-foreground/10 hover-lift">
        ...
        <div className="relative max-w-4xl mx-auto">
            ...
            >
              {testimonials.map((testimonial, index) => (
                <div key={index} className="w-full flex-shrink-0 px-4">
                  <div className="bg-card text-card-foreground rounded-2xl p-8 shadow-card border border-border/50">
                    {/* Large gradient quote */}
                    <div className="w-12 h-12 rounded-lg gradient-primary flex items-center justify-center mb-6 opacity-80">
                    ...
                    <p className="text-lg mb-6 text-foreground/90 leading-relaxed">"{testimonial.text}"</p>
          ...
          <Button
            ...
            size="icon"
            onClick={goToPrev}
            className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 bg-card border-border hover:bg-accent hidden md:flex shadow-card"
          >
          ...
          <Button
            ...
            size="icon"
            onClick={goToNext}
            className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 bg-card border-border hover:bg-accent hidden md:flex shadow-card"
          >
            ...
            {testimonials.map((_, index) => (
              <button
                ...
                className={`h-2 rounded-full transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
                  index === currentIndex
                    ? "bg-primary w-8"
                    : "bg-secondary-foreground/30 hover:bg-secondary-foreground/50 w-2"
                }`}
        ...
        <ScrollReveal>
          <div className="text-center mt-12">
            ...
            <Button asChild className="gradient-primary text-primary-foreground shadow-elegant hover-lift">
