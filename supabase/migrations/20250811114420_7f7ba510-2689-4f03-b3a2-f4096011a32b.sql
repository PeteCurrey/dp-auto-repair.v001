...
-- Recreate policies to avoid IF NOT EXISTS incompatibilities
-- Pageviews policies
DROP POLICY IF EXISTS "Anyone can insert pageviews" ON public.web_pageviews;
...
DROP POLICY IF EXISTS "Employees can read pageviews" ON public.web_pageviews;
CREATE POLICY "Employees can read pageviews"
ON public.web_pageviews
FOR SELECT
TO authenticated
USING (get_user_role() = ANY (ARRAY['employee'::text, 'admin'::text]));

DROP POLICY IF EXISTS "Employees can delete analytics pageviews" ON public.web_pageviews;
CREATE POLICY "Employees can delete analytics pageviews"
ON public.web_pageviews
FOR DELETE
TO authenticated
USING (get_user_role() = ANY (ARRAY['employee'::text, 'admin'::text]));
...
-- Events policies
DROP POLICY IF EXISTS "Anyone can insert events" ON public.web_events;
...
DROP POLICY IF EXISTS "Employees can read events" ON public.web_events;
...
DROP POLICY IF EXISTS "Employees can delete analytics events" ON public.web_events;
CREATE POLICY "Employees can delete analytics events"
ON public.web_events
FOR DELETE
TO authenticated
USING (get_user_role() = ANY (ARRAY['employee'::text, 'admin'::text]));
